...
html, body, .stApp { 
    background: #0F172A !important;  /* softer dark blue */
    color: #E2E8F0;
...
.rec-card { background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.07); border-left: 3px solid var(--accent, #38BDF8); border-radius: 12px; padding: 1.5rem; margin-bottom: 1rem; }
.rec-header { display: flex; align-items: center; gap: 0.75rem; margin-bottom: 0.75rem; }
.rec-icon { font-size: 1.2rem; }
.rec-title { font-size: 0.95rem; font-weight: 600; color: #E2E8F0; }
.rec-body { font-size: 0.85rem; color: #E2E8F0; line-height: 1.65; margin-bottom: 1rem; }
.rec-metrics { display: flex; gap: 1.5rem; }
.rec-metric-val { font-size: 1.1rem; font-weight: 700; color: #4ADE80; font-family: 'DM Mono', monospace; }
.rec-metric-label { font-size: 0.72rem; color: #CBD5E1; }
...
# ==========================================
# DATA LOADING (HISTORICAL & PREDICTIVE)
# ==========================================
@st.cache_data
def load_data():
    files = [
        'BI_Mess_Peak_Hours.csv', 
        'BI_Energy_Anomalies.csv', 
        'BI_Space_Utilization.csv',
        'BI_Adv_Exam_Stress_Index.csv', 
        'BI_Adv_Flight_Risk.csv', 
        'semester_wifi_usage_24_weeks.csv',
        'BI_Full_Student_Ledger.csv',
        'FAST_predicted_attendance.csv', 
        'wifi_predictions_next_24h.csv',
        'electricity_predictions_next_24h.csv', 
        'predicted_messhrs.csv'
    ]
